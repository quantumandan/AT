...
Flows are an extension of a directed graph's path algebra to include sets of paths. Let $V=\{1,2,3\}$ and $E=\{(1,2), (2,3), (3,2), (3,1)\}$ be the vertex and edge sets defining directed graph $G=(V,E)$. For $a=(1,2)$ and $b=(2,3)$, the product is given by $a \times b \mapsto (1,2)(2,3)$. In other words, multiplication of two edges maps to a longer path in the space of walks generated by the concatenation of head to tail matching paths. Should a head to tail matching exist but the concatenated edges do not form a valid walk on the graph, the product is $0:=\empty$

* $c=(3, 2) \in E$, the product $c\times a=(3,2)\times(1,2)= 0$.
...
For $\alpha \in 2^V$ the push mapping $F_+:2^V \to 2^E$ takes a directed graph's path and yields all successor edges,

$F_+(\alpha)=\sum_{j \in \epsilon_+ (\hat{h}\alpha)}(\hat{h}\alpha,j)$
...
For $\alpha \in 2^V$ the pull mapping $F_-:2^V \to 2^E$ takes a directed graph's path and yields all predecessor edges,

$F_-(\alpha)=\sum_{j \in \epsilon_- (\hat{t}\alpha)}(j,\hat{t}\alpha)$
...
$\operatorname{Cyc}_n(G)=F_-^p(\epsilon_+(x))F_+^q(\epsilon_-(x))\cong [\operatorname{tr}(W_G^n)]$
...
The transition space of a Turing machine is represented by the sequence of states the successor function takes on some initial data x. could make after $n$ applications of the Turing successor function corresponds to the set of $𝑛$-length sequences $[x]_{n}=\{(x_0, x_1, ..., x_k\} | k \in \mathbb{Z}_{n}\}$
...
*Consequence 1* and *Consequence 2* imply that intrinsic complexity has a lower semi-continuous spectrum, even if information is quantized/digitized.
...
The complexity class of decision problems with an irrational Turing Sequence is called $\mathbf{HARD_{AF}}$, where by Dedekind cut construction, $\mathbf{HARD_{AF}}$ can be shown to be the algebraic completion of almost finite rational Turing Sequences. Remember that the set of algorithms with polynomial time complexity $\mathbf{P}$ lies inside the almost finite polynomial ring generated by the rational Turing Sequences. As $\mathbf{HARD_{AF}}$ labels both the complexity class and the set of solutions in this class, so it is the algebraic closure of the polynomially expressible algorithms $\mathbf{A_P}$, and necessarily $\mathbf{P}$ and $\mathbf{HARD_{AF}}$ are disjoint.

The symmetric difference of disjoint sets equals their union, therefore $\mathbf{P}\triangle\mathbf{HARD_{AF}}=\mathbf{P}\cup\mathbf{HARD_{AF}}$. Similarly, it is clear that $\mathbf{NP}$ and $\mathbf{HARD_{AF}}$ are not disjoint and the symmetric difference of two overlapping sets equals the complement of their intersection, thus $\mathbf{NP}\triangle\mathbf{HARD_{AF}}=(\mathbf{NP}\cap\mathbf{HARD_{AF}})^C=\mathbf{NP}\cup\mathbf{HARD_{AF}}-\mathbf{NP}\cap\mathbf{HARD_{AF}}$. As the symmetric difference is distributive over set intersection and the operation of intersecting sets is commutative and associative, it therefore follows that 

$\mathbf{P}\cap(\mathbf{NP}\triangle\mathbf{HARD_{AF}})=\mathbf{P}\cap(\mathbf{NP}\cup\mathbf{HARD_{AF}}-\mathbf{NP}\cap\mathbf{HARD_{AF}})$

Reducing, $\mathbf{P}\cap(\mathbf{NP}\triangle\mathbf{HARD_{AF}})=\mathbf{P}\cap\mathbf{NP}\cup\empty$.

$\mathbf{P}\cap\mathbf{NP}=\empty$. In other words, in the algorithmic topology, $\mathbf{P}\ne\mathbf{NP}$. -->
...
for every bounded continuous homomorphism $H$ and $m\lt n$. However in the limit that $m$ goes to $n$
...
**Lesson 2:** Algorithmic topology says that complexity is relative but difficulty is conserved.

Similar to relatively -- where matter tells spacetime how to curve and spacetime tells matter how to move -- the expressive power of a programming language interacts with the difficulty of a problem to produce the complexity of an algorithm.
